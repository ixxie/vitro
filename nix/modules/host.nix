...
in {
  ...
  config = mkIf cfg.enable {
    ...
    # nftables: envs can ONLY reach the proxy
    networking.nftables = {
      enable = true;
      tables.vitro = {
        family = "inet";
    ...
    # DNS for VMs — env names resolve to VM IPs
    services.dnsmasq = {
      enable = true;
      settings = {
    ...
    # Host config JSON — read by mkEnv at nix eval time
    environment.etc."vitro/host-config.json".text = hostConfig;

    # Allow git operations on env repos (owned by uid 1000, accessed by root via SSH/vitro-services).
    # Scoped to the env tree — host code never touches git outside this prefix.
    ...
    # Optional GC timer for stopped envs
