# vitro.lib.mkCell — creates a NixOS microVM configuration for a cell.
#
# Used by the per-cell wrapper flake generated at runtime by `vitro up`.
    ...
    modules = [
      ...
      (vitro + "/nix/modules/guest/base.nix")
      {
        ...
        microvm = {
          ...
          # Anchor the writable overlay on the /var volume (sized via
          # vm.varSize), not on the small rootfs. Without this, new
          # nix paths (e.g. nix-shell deps in flows) fill up the 2 GB
          # rootfs and fail with "no space left on device".
